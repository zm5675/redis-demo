...
@WebServlet("/UserShopCartManageServlet")
public class UserShopCartManageServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        String sql = "select a.*,b.UserName,c.GoodName,c.GoodUnitPrice from ShopCart a inner join USER b on a.UserID=b.id inner join Good c on a.GoodID=c.id where 1 = 1 and a.UserID='"+userId+"' and OrderID is null";
        StringBuilder sb = new StringBuilder(sql);
        Set<String> keySet = condition.keySet();       
        List<Object> params = new ArrayList<Object>();
        if(isDel==null&&totalCount1==null) {
        ...
        List<ShopCart> list = template.query(sql,new BeanPropertyRowMapper<ShopCart>(ShopCart.class),params.toArray());
        PageBean<ShopCart> pb = new PageBean<ShopCart>();
        pb.setCurrentPage(Integer.parseInt(currentPage));
        pb.setRows(Integer.parseInt(rows));
        String sql1 = "select count(*) from ShopCart where 1 = 1 and UserID='"+userId+"' and OrderID is null";
        ...
        request.getRequestDispatcher("UserShopCartManage.jsp").forward(request,response);
    }
